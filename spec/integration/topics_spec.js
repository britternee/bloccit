...
describe("routes : topics", () => {
    ...
    describe("POST /topics/create", () => {
        ...
        it("should not create a new topic", (done) => { //members can't create topics
            request.post(options,
                (err, res, body) => {
                    Topic.findOne({where: {title: "blink-182 songs"}})
                    .then((topic) => {
    ...
    describe("POST /topics/:id/destroy", () => {
        ...
        it("should not delete the topic with the associated ID", (done) => { //members can't delete topics
            Topic.all()
            .then((topics) => {
                const topicCountBeforeDelete = topics.length;
                expect(topicCountBeforeDelete).toBe(1);
                request.post(`${base}${this.topic.id}/destroy`, (err, res, body) => {
                    Topic.all()
                    .then((topics) => {
                        expect(topics.length).toBe(topicCountBeforeDelete); //confirm no topics were deleted
                        done();
                    })
                });
            })
    ...
    describe("GET /topics/:id/edit", () => {
        ...
        it("should not render a view with an edit topic form", (done) => {
            request.get(`${base}${this.topic.id}/edit`, (err, res, body) => {
                ...
                expect(body).toContain("JS Frameworks"); //confirm redirect to show topics
                done();
            });
        });
    });
